...
const Inbox = () => {
        ...
        if (cookieData.message !== "success") {
            ...
            if (dataIncoming !== null) {
                for (var i = 0; i < dataIncoming.length; i++) {
                    ...
                    if (parsedJSON.total !== 0) {
                        consentIncoming.push(dataIncoming[i]);
                    }
                }
            } else {
                setConsentIncoming([]);
    ...
    const getProviderByToken = async (token) => {
        let url =
            `${resolveURL("healthcare-provider")}/healthcare-provider/getID/` +
            token;
        const response = await fetch(url, {
            method: "GET",
            mode: "cors",
                        ...
                        <div className="requests-page">
                            ...
                            <div className="consent-list">
                                {consentRequests.map((consent_elem, idx) => {
                                    return (
                                        <div
                                            className="consent-request-elem"
                                            id={consent_elem.ID}
                                            key={idx}
                                        >
                                            <div className="information-div">
                                                ...
                                                <p id="information">
                                                    {consent_elem.datecreated}
                                                </p>
                                            </div>
                                            <button
                                                id="view"
                                                onClick={() => {
                                                    viewForm(consent_elem.ID);
                                                }}
                                            >
                                                View
                                            </button>
