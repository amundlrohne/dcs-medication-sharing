...
const MedicationRequestForm = () => {
        ...
        if (
            ...
            if (validateDrugName.message === "valid") {
                let medication = {
                    ...
                    dosagesequence: parseInt(dosageSequence),
    ...
    const deleteMedication = (med_elem) => {
        let filtered = medications.filter(
            (elem) => JSON.stringify(elem) !== JSON.stringify(med_elem)
        );
    ...
    const handleSearch = async (e) => {
        ...
        if (input.length > 2) {
            ...
            data.length === 0
                ? showDrugListEmpty(true)
                : showDrugListEmpty(false);
        } else {
            setDrugList([]);
            showDrugList(false);
        }
    ...
    return (
        <div>
            <div className="medication-request-form">
                ...
                {drugListEmpty && <p id="drug-list-empty">No drugs found</p>}

                {drugListVisibility && drugList && (
                    <div className="drug-list">
                        {drugList.map((drug, idx) => {
                            return (
                                <li onClick={() => selectDrug(drug)} key={idx}>
                                    {drug}
                                </li>
                            );
                        })}
                ...
                <label htmlFor="effectiveDateTime">
                    ...
                    <input
                        ...
                        placeholder="Date of when pill was first prescribed (i.e. yyyy-mm-dd)"
                ...
                <label htmlFor="dosageSequence">
                    ...
                    <input
                        type="number"
                        name="dosageSequence"
                        placeholder="Sequence of how many times patient takes medication (i.e. 1)"
                        value={dosageSequence}
                        onChange={(e) => setDosageSequence(e.target.value)}
                    ></input>
                </label>
            ...
            {medicationListVisibility && medications && (
                <div className="added-medications">
                    ...
                    {medications.map((med_elem, idx) => {
                        return (
                            <div
                                className="medication-elem"
                                id={med_elem.medication + "-" + idx}
                                key={idx}
                            >
                                <p id="information">
                                    {med_elem.effectivedatetime} |{" "}
                                    {med_elem.medication}
                                </p>
                                <button
                                    id="delete"
                                    onClick={() => {
                                        deleteMedication(med_elem);
                                    }}
                                >
                                    Delete
                                </button>
            ...
            {submitVisbility && (
                <button
                    id="submit-medical-record"
                    onClick={() => {
                        submitMedicationHistory();
                    }}
                >
                    {submitText}
                </button>
